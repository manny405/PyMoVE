...
from pymove.molecules.topological_fragments import TopologicalFragments,\
                                              TopologicalFragmentsModel
                                              
import matplotlib.pyplot as plt



class FragmentImage(BaseDriver_):
    def __init__(self, TopologicalFragments_object=TopologicalFragments()):
        self.bn = TopologicalFragments_object
        
    
    def calc_struct(self, struct):
        raise Exception("Must pass in as a dictionary and not an individual structure.")
    
    
    def calc_dict(self, struct_dict, name_list=[], figname="",
    ...
    def _find_fragments(self, struct):
        ...
        """
        fragments,counts = self.bn.calc_struct(struct)
        g = self.bn._build_graph(struct)
