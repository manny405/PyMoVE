...
class TopologicalFragments(BaseDriver_):
    """
    ...
    def _calc_neighbors(self, g):
        ...
        which was declared when TopologicalFragments was initialized. Ordering
...
class TopologicalFragmentsModel():
    """
    Manages the construction of a regularized topological fragment model. Also
    ...
    Usage
    -----
    0. Initialize a TopologicalFragments class. Initialized the constructor
       class with the TopologicalFragments and the list of extra properties 
    ...
    Arguments
    ---------
    bn: TopologicalFragments object
        Initialized TopologicalFragments object.
    ...
    def fit_model(self, regressor, r=True, test=True, weighted=False):
        """
        ...
        if not weighted:
            ...
            y  = self.target_df.values.ravel()
            regressor.fit(temp_df.values/y[:,None], 
                          y/y)  
#            regressor.fit(temp_df.values, self.target_df.values, 
#                          1/self.target_df.values.ravel())
    ...
    def set_train(self, struct_dict):
        ...
        for struct_id,struct in struct_dict.items():
            ...
            struct.set_property("topological_fragments_fragments", f)
            struct.set_property("topological_fragments_counts", c)
        ...
        # Building final neighborhood dataframe from unique topological fragments
...
#            f = struct.properties["topological_fragments_fragments"]
#            c = struct.properties["topological_fragments_counts"]
    ...
    def regularize(self, tol=10, unique=True):
        ...
        if len(self.neighborhood_df) == 0:
            raise Exception("Please call "+
                    "construct_topological_fragments_model.set_train before you "+
                    "call regularize.")
        drop_list = []
    ...
    def model_complexity_analysis(self, regressor, choose_n_features=-1,
        ...
        if len(neighborhood_df_train) == 0:
            if len(self.neighborhood_df) == 0:
                raise Exception("Called TopologicalFragmentsModel.model_complexity_analysis "+
        ...
        if len(self.neighborhood_df_r) == 0:
            if len(self.neighborhood_df) == 0:
                raise Exception("The user called "+
                   "construct_topological_fragments_model.{} ".format(method_name) +
    ...
    def plot_hist(self, neighborhood_df, 
        ...
        """
        Plots a histogram of the frequency of topological fragments identified
        ...
        Regressor: regressor object
            ...
            only the N first most important topological fragments.
        most_important: int
