...
def get_rot_symmetry(mol, max_rot=10, tol=0.1, euler=False):
    ...
    ### Make sure geo is aligned with principle axes before rotations
    temp_mol = copy.deepcopy(mol)
    ele = temp_mol.geometry["element"]
    for i in range(10):
        geo = temp_mol.get_geo_array()
        principal_axes = get_principle_axes(temp_mol)
