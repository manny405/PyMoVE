...
class Structure(object):
    ...
    def translate(self, trans):
        ...
        """
        self.geometry["x"] += trans[0]
        self.geometry["y"] += trans[1]
        self.geometry["z"] += trans[2]
    ...
    def get_sub(self, idx, lattice=False):
        """
        ...
        sub = Structure.from_geo(geo[idx], self.geometry["element"][idx])
        sub.properties["Parent_ID"] = self.struct_id
        if lattice:
            sub.properties["lattice_vector_a"] = self.properties["lattice_vector_a"]
            sub.properties["lattice_vector_b"] = self.properties["lattice_vector_b"]
            sub.properties["lattice_vector_c"] = self.properties["lattice_vector_c"]
        sub.get_struct_id()
        return sub
...
#        except: self.properties[key] = value
    ...
    def loads(self, json_string):
        ...
        if "input_ref" in data_dictionary:
            del(data_dictionary["input_ref"])
