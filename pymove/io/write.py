...
pymove_file_formats = ['json','geometry', 'geo', 'aims', "MBD", "mbd"]
...
def write(path, struct_obj, file_format='json', combined=False, overwrite=False):
    ...
    file_format: str or list of str
        Any of the pymove_file_formats or ase_file_formats are acceptable
    overwrite: bool
    ...
    if file_format == 'json':
        ...
        try: ase_all_formats[file_format]
            ...
            raise Exception("Unsupported file format. "+
              "Please try one of pymove file formats: {} \n"
              .format(pymove_file_formats) +
...
def output_struct(file_path, struct, overwrite=False):
    ...
    """
    struct.properties["pymove"] = "Structure file was written using pymove."
...
def file_ext(file_path, file_ext):
    ...
    ## Check if current file extension is known by pymove
    temp_file_ext = file_name.split(".")[-1]
    if temp_file_ext in pymove_ext:
