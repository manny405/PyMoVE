...
## All possible extensions allowed in pymove
pymove_ext = ["in", "json","next_step","cif", "pt"]
pymove_ext += [x for x in ase_extension2format.keys()]
...
def check_format(file_format):
    """
    Check if file_format is accepted by pymove
...
def check_file_type(file_path, verbose=True):
    ...
    if ext not in pymove_ext:
        if verbose:
            print("File extension \".{}\" was not recognized by pymove."
                   .format(ext))
        return ""
    else:
