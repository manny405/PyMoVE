...
from pymove.io.check import check_ext,check_format,format2extension,check_dir
...
class StructDict(dict):
    ...
    def append(self, struct, struct_id=""):
        ...
        Arguments
        ---------
        struct: pymove/ASE/Pymatgen
            Can be a Structure object from pymove, ASE, or Pymatgen
        struct_id: str
        ...
        ## Now handle struct_id for cases where object was not pymove
        if len(struct_id) > 0:
...
class StructDictMongo():
    ...
    pymove API. This means that any analysis that people will want to do using 
