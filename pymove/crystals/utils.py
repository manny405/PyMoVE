...
def standardize(struct):
    """
    Standardize using internal method in the pymatgen symmetry analyzer. Please
    note that this is not guaranteed to keep molecules whole in the Structure. 
    If whole molecules are required, the user may call whole_molecules after
    standardize. 
    
    """
    pstruct = struct.get_pymatgen_structure()
    ### Very tight tolerances used in order to reduce the impact of the 
    ### structure.refine routines
    sga = SpacegroupAnalyzer(pstruct, symprec=1e-6, angle_tolerance=1e-6)
    standard_struct = sga.get_conventional_standard_structure(
            international_monoclinic=False)
    standard_struct = Structure.from_pymatgen(standard_struct)
    struct.reset_lattice_vectors(standard_struct.get_lattice_vectors())
    struct.geometry = standard_struct.geometry
    return struct
...
def preprocess(struct, 
    ...
    Please note that some uncommon molecular crystals may be guaranteed to fail. 
    These are crystals that have infinite periodic chains of covalent bonds. 
...
#    from pymove.analysis import get
    from pymatgen.symmetry.analyzer import SpacegroupAnalyzer
