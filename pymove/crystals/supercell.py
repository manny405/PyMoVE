...
from pymove.crystals.lebedev import lebedev_5
from pymove.crystals.utils import get_molecules,preprocess
from pymove.molecules import com

from pymove.io import write
...
class Supercell(BaseDriver_):
    ...
    def calc_struct(self, struct):
        self.struct = struct
        if self.preprocess:
            self.struct = preprocess(struct, bonds_kw=self.bonds_kw)
        if self.track_molecules:
            self.molecule_idx = get_molecules(struct, 
    ...
    def construct_supercell(self, struct=None):
        ...
        for offset in temp_molecule_idx_offset:
...
class SupercellSphere(BaseDriver_):
    ...
    correction: bool
        ...
        used. 
    preprocess: bool
        If the structure should be preprocessed before constructing supercell. 
