...
perform SIMD type calculations using pymove calculations. 

Requirements for pymove calculation classes to use with ParallelCalc:
...
from pymove.io.write import check_dir
from pymove.libmpi.base import _NaiveParallel
...
class ParallelCalc(_NaiveParallel):
    def __init__(self, 
        ...
        self.pymove_class = pymove_class
    ...
    def _calc_write(self):
        ...
        for file_path in my_files:
            struct = read(file_path)
            self.pymove_class.calc_struct(struct)
            self.pymove_class.write(self.output_path,
                                    file_format=self.file_format,
                                    overwrite=self.overwrite)
...
#    from pymove.motif.classify_motif import MotifClassifier
#    from pymove.motif.utils import MoleculesByIndex
#    
...
#class ParallelCalc():
#    def __init__(self, struct_path, pymove_class):
#        """
